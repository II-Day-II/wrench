...
namespace Wrench
{
	...
	ShaderKernel Shader::compile(const char* entry_point)
	{
	...
	Shader load_shader(std::shared_ptr<VulkanCtx> &ctx, const char* name, const char* filename)
	{
		...
		Slang::ComPtr<slang::IModule> slang_module{ slang_session->loadModuleFromSource(name, filename, nullptr, load_diagnostics.writeRef()) }; // TODO: get compiler diagnostics and reflection data
		...
		Shader ret;
		ret.m_ctx = ctx;
		ret.slang_session = slang_session;
